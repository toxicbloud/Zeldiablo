    ...
    public void dessiner(BufferedImage image) {
        ...
        for (int x = 0; x < cases.length; x++) {
            for (int y = 0; y < cases[x].length; y++) {
                ...
                int posX = (x-(jeu.getCam().getPos().x/Labyrinthe.TILE_SIZE))*TILE_SIZE + w/2 - TILE_SIZE/2;
                int posY = (y-(jeu.getCam().getPos().y/Labyrinthe.TILE_SIZE))*TILE_SIZE + h/2 - TILE_SIZE/2;
        ...
        for(Entite e: jeu.getEnnemis()) {
            ...
            posX = (e.getPos().x-(jeu.getCam().getPos().x/Labyrinthe.TILE_SIZE))*TILE_SIZE + w/2 - TILE_SIZE/2;
            posY = (e.getPos().y-(jeu.getCam().getPos().y/Labyrinthe.TILE_SIZE))*TILE_SIZE + h/2 - TILE_SIZE/2;
            g.fillOval(posX, posY, TILE_SIZE, TILE_SIZE);
        }
    }
